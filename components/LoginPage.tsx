...
import { setMemberId, syncColor } from "@/game/identity";
...
export default function LoginPage({ onSuccess }: { onSuccess: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // The password identifies which member this is
        const { memberId } = await res.json();
        localStorage.setItem(SESSION_KEY, "true");
        setMemberId(memberId);
        // Pull their saved colour before the scene reads it
        await syncColor(memberId);
        onSuccess();
      } else {
        setError(true);
        setPassword("");
